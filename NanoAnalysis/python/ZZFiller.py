...
class ZZFiller(Module):
    def __init__(self, runMELA, bestCandByMELA, isMC, year):
        ...
        # If set to True, ZZ candidates are stored as a nanoAOD collection, so that several candidates per event can be saved (for eg. ID optimization studies)
        # Otherwise, a flat list of variables is saved for the best candidate only. This option is deprecated and will possibly be removed.
        self.storeCandsAsCollection = True
        # Write only the best candidate in each event
        self.storeBestCandOnly = False

        # Lepton preselection used to build Z and ZZ candidates
        if self.storeCandsAsCollection and not self.storeBestCandOnly:
            # Relaxed muon preselection for ID studies: fully relax muon ID. Electron ID is unchanged.
            self.leptonPresel = (lambda l : (abs(l.pdgId)==13 and l.pt>5 and abs(l.eta) < 2.4) or (abs(l.pdgId)==11 and l.ZZFullId)) 
        else : 
            # Standard full lepton selection
        ...
        # Add flags for muon selection studies. Each Flag will be set to true for a candidate if all of its muons pass the specified ID.
        ...
        # Set which of the above criteria is applied when searching for the best candidate
        self.idForBestCandSel = 0
        if self.muonIDs[self.idForBestCandSel]["name"] != "ZZFullSel" :
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        if self.storeCandsAsCollection: # Use the nanoAOD collection format
        ...
        else: # store only the best candidate as a flat set of variables (deprecated)
            ...
            self.out.branch("Z2_l1Idx", "I")
            self.out.branch("Z2_l2Idx", "I")
        ...
        if len(Zs) >= 2:
            for iZ,aZ in enumerate(Zs):
                for jZ in range(iZ+1, len(Zs)):
                    ...
                    # Set index of best cand among those passing the standard selection
                    if ZZ.passId[self.idForBestCandSel] :
                        if bestCandIdx<0 or self.bestCandCmp(ZZ,ZZs[bestCandIdx]) < 0:
                            bestCandIdx = len(ZZs)

                    if self.DEBUG: print("ZZ:", len(ZZs),  (Z1.p4+Z2.p4).M(), ZZ.Z1.M, ZZ.Z2.M, ZZ.Z2.sumpt(), ZZ.finalState(), p_GG_SIG_ghg2_1_ghz1_1_JHUGen, p_QQB_BKG_MCFM, ZZ.KD, ZZ.passId[self.idForBestCandSel])
                    ZZs.append(ZZ)
            ...
            if self.storeBestCandOnly: # keep only the best cand as single element
                ZZs = [ZZs[bestCandIdx]]
                bestCandIdx = 0


            if self.storeCandsAsCollection:
                ...
                ZZCand_Z1l1Idx = [-1]*len(ZZs) # FIXME to be implemented
                ...
                # Store only the best ZZ candidate (among all ZZs, ie those passing the self.leptonPresel selection)
                bestZZ = min(ZZs, key = cmp_to_key(self.bestCandCmp))

                Z1 = bestZZ.Z1
                Z2 = bestZZ.Z2
                ZZ_mass = bestZZ.p4.M()
                if self.DEBUG: print("bestZZ=", ZZ_mass, Z1.M, Z2.M)

                Dbkgkin = 0.
                if self.runMELA:
                    Dbkgkin = self.lib.D_bkg_kin(c_float(bestZZ.p_GG_SIG_ghg2_1_ghz1_1_JHUGen), c_float(bestZZ.p_QQB_BKG_MCFM), c_int(int(bestZZ.finalState())),c_float(ZZ_mass)) #including c-constants

                ### Fill output tree
                self.out.fillBranch("ZZ_mass", ZZ_mass)
                self.out.fillBranch("ZZ_massPreFSR", ZZ.massPreFSR())
                ...
                if self.isMC:
                    w_dataMC = self.getDataMCWeight(ZZ.leps()) 
